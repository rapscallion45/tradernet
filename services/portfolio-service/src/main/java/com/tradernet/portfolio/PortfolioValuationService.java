...
import com.tradernet.portfolio.dto.PortfolioAssetDto;
...
/**
 * Values current net positions from cached market snapshots.
 */
@Stateless
    ...
    public PortfolioSummaryDto buildSummary(
        ...
        for (Map.Entry<String, PortfolioPosition> entry : positions.entrySet()) {
            ...
            final double fallbackPrice = position.getLastKnownPrice() > 0.0
                ...
                : averageCostRaw;
            final double currentPriceRaw = marketBarProvider.getLatestCachedPrice(symbol).orElse(fallbackPrice);
